...
const Nav = () => {
    ...
    return (
        <nav aria-label="Main Navigation" className='bg-[var(--nav-backgroundcolor)]'>
            <div className={`flex flex-row justify-between items-center px-4 xl:px-12 py-2 2xl:mx-auto 2xl:w-full 2xl:max-w-[2300px] ${expanded ? 'relative' : ' '}`}>
                <ul onClick={handleToggleMenu} className={` xl:flex flex-row flex-wrap gap-4 text-xl font-quicksand  ${expanded ? ' absolute top-0 left-0 z-50 bg-[var(--nav-backgroundcolor-dark)] pt-14 pb-8 px-8 w-full flex flex-col justify-center items-center' : 'hidden'}`}>
